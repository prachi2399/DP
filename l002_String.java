public class l002_String{
    ...
    public int minDistance(String s, String t, int N, int M, int[][] dp){
        for(int n=0;n<=N;n++){
            for(int m=0;m<=M;m++){  
                ...
                int insert = dp[n][m-1];//minDistance(s,t,n,m-1,dp);
                int delete = dp[n-1][m];//minDistance(s,t,n-1,m,dp);
                int replace = dp[n-1][m-1];//minDistance(s,t,n-1,m-1,dp);
    ...
    public boolean isMatch(String s, String p) {
        ...
        for(int[] d:dp) Arrays.fill(d,-1);
        return isMatch(s,p,n,m,dp)==1?true:false;
    }
